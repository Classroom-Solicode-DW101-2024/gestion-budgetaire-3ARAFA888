...
function totalIncomesByCategory($categoryName,$pdo){
    $sql = "SELECT SUM(montant) AS total FROM transactions JOIN categories ON 
    transactions.category_id = categories.id WHERE categories.type = 'revenu' AND categories.nom = :categoryName";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
...
function totalExpensesByCategory($categoryName,$pdo){
    $sql = "SELECT SUM(montant) AS total FROM transactions JOIN categories ON 
    transactions.category_id = categories.id WHERE categories.type = 'depense' AND categories.nom = :categoryName";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            ':categoryName' => $categoryName,
        ]);
